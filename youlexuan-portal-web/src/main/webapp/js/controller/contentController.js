//内容控制层
app.controller('contentController', function ($scope, $controller, contentService, contentCategoryService, searchService) {
    ...
    $scope.searchLike = function () {
        $scope.getKeyWords();
        // $scope.searchMap.keywords = $scope.keywordsList;
        // alert(Cookies.get('keywords'))
        if (Cookies.get('keywords') != null && Cookies.get('keywords') != 'undefined') {
            searchService.searchLike(Cookies.get('keywords')).success(function (resp) {
                $scope.likeMap = resp;
            })
        }
    }
    ...
    $scope.getKeyWords = function () {
        // ,{ expires: -1}
        //     Cookies.set('keywords', $scope.keywords);
        if (Cookies.get('keywords') != null && Cookies.get('keywords') != 'undefined') {
            $scope.keywordsList = Cookies.get('keywords').split(',');
            // $scope.keywordsList.push(key.toString());
    ...
    // $scope.itemCatsList = [{'id':'','list':{'id':'','list':{'id':'','list':{},'name':'','parentId':'','typeId':''},'name':'','parentId':'','typeId':''},'name':'','parentId':'','typeId':''}];
